...
def test_animate_map_timespan_wiring(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda source, site, start, end, step: fake_radars)
    ...
    out = anim_mod.animate_map_timespan(
        "nexrad", "KTLX", start, end, step, field='reflectivity',
        resolution='50m', draw_coastline=False)
...
def test_animate_map_timespan_template_resolves_title(monkeypatch):
    ...
    monkeypatch.setattr(
        pyart.io, "read_time_span",
        lambda *a, **k: fake_radars)
    monkeypatch.setattr(
        anim_mod, "animate_map_ppi",
        lambda radars, field, **kw: calls.setdefault("kw", kw))

    anim_mod.animate_map_timespan(
        "nexrad", "KTLX", datetime(2020, 6, 1), datetime(2020, 6, 1, 1),
        timedelta(minutes=5), template="timespan")
