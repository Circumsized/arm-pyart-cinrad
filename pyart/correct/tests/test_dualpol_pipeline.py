...
def _make_dualpol_fields():
    ...
    shape = (3, 100)
    return {
        zdr: {'data': np.ma.array(np.full(shape, 1.5), mask=False),
              'long_name': 'Differential Reflectivity'},
        phi: {'data': np.ma.array(np.full(shape, 60.0), mask=False),
              'long_name': 'Differential Phase'},
        ldr: {'data': np.ma.array(np.full(shape, -30.0), mask=False),
              'long_name': 'Linear Depolarization Ratio'},
    }


@pytest.mark.parametrize('filename,expected_band', [
    # S-band family
...
def test_read_cinrad_metadata_contract(filename, expected_band):
    ...
    assert inferred == expected_band, (
        'band inference regressed for {0!r}: got {1!r}, expected {2!r}'
        .format(filename, inferred, expected_band))

    r = _make_fake_radar(band_metadata={'radar_band': inferred},
    ...
    assert not user_warnings, (
        '_resolve_band emitted UserWarning despite correct metadata: ' +
...
def test_resolve_band_falls_back_to_c_with_warning_when_metadata_missing():
    ...
    assert band == 'C'
    user_warnings = [w for w in caught if issubclass(w.category, UserWarning)]
    assert any('defaulting to' in str(w.message) for w in user_warnings), (
...
def test_calibrate_dualpol_uses_metadata_band_not_fallback():
    ...
    real_band_params = dict(cband_sband.BAND_PARAMS)
    ...
    try:
        r = _make_fake_radar(band_metadata={'radar_band': 'S'},
                             fields=fields)
    ...
    assert selected[0] == 'S', (
        'calibrate_dualpol resolved band {0!r} (expected S); '
        'BAND_PARAMS entry used: {1}'.format(
            selected[0], cband_sband.BAND_PARAMS[selected[0]]))
...
def test_pipeline_smoke_test_writes_correct_metadata_for_all_formats():
    ...
    """
    from pyart.io.cinrad_bridge import _infer_band_from_filename
    from pyart.correct.cband_sband import _resolve_band, BAND_PARAMS
    ...
    for filename, expected_band in docs_formats:
        inferred = _infer_band_from_filename(filename)
        assert inferred == expected_band
        assert inferred in BAND_PARAMS, (
            'inferred band {0!r} is not in BAND_PARAMS'.format(inferred))
        r = _make_fake_radar(band_metadata={'radar_band': inferred},
                             fields=_make_dualpol_fields())
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter('always')
        ...
        assert not [w for w in caught if issubclass(w.category, UserWarning)], (
            'unexpected UserWarning for {0}'.format(filename))
