...
def radars():
    ...
    for i in range(2):
        ...
        data = np.full((radar.nrays, radar.ngates), 20.0 + 5 * i, dtype="float32")
        radar.add_field("reflectivity", {"data": data, "units": "dBZ"})
        frames.append(radar)
    return frames
...
def test_animate_ppi_bad_outdir(radars, tmp_path):
    with pytest.raises(ValueError):
        animate_ppi(radars, "reflectivity", out=str(tmp_path / "nope" / "x.gif"))


@pytest.mark.skipif(
    pytest.importorskip("pyart") is None and False,
...
def test_animate_ppi_batch(radars, tmp_path):
    ...
    files = [str(tmp_path / f"frame_{i}.nc") for i in range(3)]
    for i in range(3):
        write_cfradial(files[i], radars[i % 2])
    out_dir = str(tmp_path / "batch")
    report = animate_ppi_batch(
        files, "reflectivity", out_dir=out_dir, sweep=0, vmin=0, vmax=70
    )
...
def test_animate_multi_band(radars, tmp_path):
    ...
    animate_multi_band(
        radars_by_band, "reflectivity", out=out, sweep=0, vmin=0, vmax=70
    )
    import imageio
