...
def test_c98dfile_archive_reads_dualpol_fields():
    ...
    assert np.allclose(radar.fields["differential_reflectivity"]["data"][0, 0], 1.5)
    assert np.allclose(radar.fields["cross_correlation_ratio"]["data"][0, 0], 0.9)
    assert np.allclose(radar.fields["differential_phase"]["data"][0, 0], 180.0)
    assert np.allclose(radar.fields["specific_differential_phase"]["data"][0, 0], 0.5)
...
def test_c98dfile_archive_metadata():
    ...
    assert radar.instrument_parameters["unambiguous_range"]["data"][0] == 150000
