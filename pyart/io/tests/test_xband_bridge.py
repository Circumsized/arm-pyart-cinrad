...
from pyart.io.cinrad_bridge import (
    is_xband_filename, read_xband, _detect_reader, _read_via_pywr)


def test_is_xband_filename():
    assert is_xband_filename(
        'Z_RADR_I_ZA601_20240415183600_O_DOR_AXPT0364_CRA_FMT.bin')
    assert is_xband_filename(
        'Z_RADR_I_ZBJ02_20210815155836_O_DOR_DXK_CAR.bin')
    assert is_xband_filename('XAD_some_file.bin')
    assert not is_xband_filename(
...
def test_read_xband_routes_via_mocked_cinrad():
    ...
    with mock.patch(
            'pyart.io.cinrad_bridge._detect_reader',
            return_value=lambda: fake_obj), \
            mock.patch(
...
def test_read_xband_falls_back_to_pycwr():
    ...
    with mock.patch('pyart.io.cinrad_bridge._detect_reader',
                    return_value=broken_reader), \
            mock.patch('pyart.io.cinrad_bridge._read_via_pywr',
                       return_value=fake_radar):
...
@pytest.mark.skipif(os.environ.get('CINRAD_TEST_FILE') is None,
                    reason='CINRAD_TEST_FILE not set')
