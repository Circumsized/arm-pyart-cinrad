...
Functions for reading Chinese S-band (CINRAD-SA) Level II Archive files.

Note: CINRAD-SA Level II files do not embed the radar site location, so the
site latitude / longitude / height must be supplied through the ``station``
argument of :func:`read_sband_archive`.
...
def read_sband_archive(
    ...
    **kwargs,
    ...
    station : tuple or None, optional
        Three float tuple giving the latitude, longitude and height of the
        S-band radar (first latitude, second longitude, third height; height
        in meters). CINRAD-SA Level II files do not store the site location,
        so provide this to correctly georeference the radar.
