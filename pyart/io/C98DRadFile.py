...
class C98DRadFile(object):
    ...
    def get_data(self, moment, cutnum=None, raw=False):
        ...
        if moment not in self.moment_headers:
            raise KeyError(
                'Unknown moment "{0}", must be in {1}'.format(
                    moment, sorted(MOMENTS_TYPE.values())))
        ...
        if cutnum is None:
            ...
            if cutnum >= self.cutnum:
                raise ValueError(
                    'Cut number must be less than {0}'.format(
                        self.cutnum + 1))
        ...
        if header['bin_length'] == 1:
            ...
            combined = np.array(data[1::2] * 256 + data[::2],
                                dtype=np.float64)
            ...
            raise ValueError(
                'Moment bin_length {0} is WRONG!'.format(header['bin_length']))
    ...
    def get_range(self, moment, cutnum):
        ...
        if moment in ['dBT', 'dBZ', 'ZDR', 'LDR', 'CC', 'QDP', 'KDP', 'Zc',
                      'HCL', 'SQI', 'CPA', 'CP', 'FLAG', 'CF']:
    ...
    def get_unambiguous_range(self):
        ...
        return [max(self.cut_info['maxi_range1'][i],
                    self.cut_info['maxi_range2'][i])
