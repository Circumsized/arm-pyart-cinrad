...
def _radar_from_arrays(metadata, time_seconds, ranges, fields,
                       az_data, el_data, sweep_number, fixed_angle,
                       nyquist_speed):
    ...
    for name, dic in fields.items():
        ...
        if dic['data'].shape != (nrays, ngates):
            raise ValueError(
                'Field {0} has shape {1}, expected ({2}, {3})'.format(
    ...
    return Radar(
        time, _range, fields,
        {'original_container': metadata['container'],
         'radar_band': 'X'},
        'ppi', latitude, longitude, altitude,
        {'data': np.arange(len(fixed_angle), dtype='int32')},
        {'data': np.array(
            len(fixed_angle) * ['azimuth_surveillance'], dtype='S')},
...
def _read_volume(raw, header, ray_layout, moment_layout, moment_codes,
                 container, expected_magic):
    """ Shared parser: validate magic, unpack rays and moment blocks. """
    size_h = struct.calcsize(_fmt(header))
    if len(raw) < size_h:
        raise ValueError('truncated {0} header'.format(container))
    hdr_vals = struct.unpack(_fmt(header), raw[:size_h])
    if hdr_vals[0] != expected_magic:
        raise ValueError('bad magic {0!r} for {1}'.format(
            hdr_vals[0], container))
    ...
    for ray in range(nrays):
        ...
        for code in moment_codes:
            ...
            if pos + ngates > len(raw):
                raise ValueError('truncated moment data for ray {0}'.format(
                    ray))
            fields_raw[code].append(
                (dtype, scale, offset,
                 np.frombuffer(raw, dtype='uint8', count=ngates, offset=pos)
                 .copy()))
            pos += ngates
    ...
    for code, name in moment_codes.items():
        ...
        for ray, (_, scale, offset, raw_gates) in enumerate(frames):
            data[ray] = _decode_uint8_moment(
                raw_gates, scale, offset, code)
        fields[name] = {'data': data, 'units': _MOMENT_UNITS.get(name)}

    metadata = {'lat': info['site_lat'], 'lon': info['site_lon'],
                'alt': info['site_alt'], 'container': container}
...
def read_xband_724xsp(filename):
    ...
    metadata, ranges, fields, az, el, info = _read_volume(
        raw, XSP724_HEADER, XSP724_RAY, XSP724_MOMENT, MOMENT_CODES,
        '724XSP', b'724X')
    return _radar_from_arrays(
        metadata, np.arange(len(az), dtype='float64'), ranges, fields,
        az, el, 0, [info['fixed_angle']], info['nyquist_speed'])
...
def read_xband_scrxd01(filename):
    ...
    with open(filename, 'rb') as f:
        raw = f.read()
    metadata, ranges, fields, az, el, info = _read_volume(
        raw, SCRXD_HEADER, SCRXD_RAY, SCRXD_MOMENT, MOMENT_CODES,
        'SCRXD-01', b'SCRX')
    return _radar_from_arrays(
        metadata, np.arange(len(az), dtype='float64'), ranges, fields,
        az, el, 0, [info['fixed_angle']], info['nyquist_speed'])