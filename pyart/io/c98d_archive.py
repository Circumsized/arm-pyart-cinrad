...
def c98dfile_archive(filename, field_names=None, additional_metadata=None,
    ...
    for raw_moment in nfile.get_moment_type:
        ...
        for cn in cutnum[1:]:
            ...
            if fndata.shape[1] < ngates:
                pad = np.full((fndata.shape[0], ngates - fndata.shape[1]),
                              np.nan)
                fndata = np.c_[fndata, pad]
            elif fndata.shape[1] > ngates:
                raise ValueError(
                    'Inconsistent gate count for moment {0}'.format(
                        raw_moment))
    ...
    _range['data'] = (
        first_gate +
        np.arange(max_ngates, dtype='float32') * gate_spacing)
    ...
    for field_name in fields:
        ...
        if data.shape[1] < max_ngates:
            ...
            fields[field_name]['data'] = np.ma.masked_invalid(
                np.c_[data, pad])
    ...
    sweep_mode['data'] = np.array(
        nsweeps * ['rhi'] if scan_type == 'rhi' else
        nsweeps * ['azimuth_surveillance'], dtype='S')

    sweep_end_ray_index['data'] = np.cumsum(nrays_per_sweep,
                                            dtype='int32') - 1
    ...
    fixed_angle['data'] = elevation_full[cut_start[cutnum_idx]].astype(
        'float32')
    ...
    unamb_rng = np.array(
        nfile.get_unambiguous_range, dtype='float32')[cutnum_idx]
    ...
    radar_calibration = {
        'zdr_calibration': {
            'data': np.array([calibration['zdr_calibration']],
                             dtype='float32'),
            'units': 'dB',
            'long_name': 'ZDR system calibration offset',
            '_FillValue': get_fillvalue()},
        'phase_calibration': {
            'data': np.array([calibration['phase_calibration']],
                             dtype='float32'),
            'units': 'degrees',
            'long_name': 'PhiDP system calibration offset',
            '_FillValue': get_fillvalue()},
        'ldr_calibration': {
            'data': np.array([calibration['ldr_calibration']],
                             dtype='float32'),
            'units': 'dB',
